...
void swaps (peminjaman *a,peminjaman *b)
{
...
void quickSort(peminjaman *buku, int left, int right)
{
    int a = left, b = right;
    int pivot = buku[(left + right) / 2].total;
    while (a <= b)
    {
        while (buku[a].total < pivot)
            a++;
        while (buku[b].total > pivot)
            b--;
        if (a <= b)
        {
            swaps(&buku[a],&buku[b]);
    ...
    if (left < b)
        quickSort(buku, left, b);
    if (a < right)
        quickSort(buku, a, right);
}
